...
class Player(pygame.sprite.Sprite):
    ...
    def load_data(self):
        ...
        file_path = os.path.join('source/data/player/',file_name)  #使用该项目绝对定位
    ...
    def load_images(self):   #载入主角的各种帧造型
        ...
        for group,group_frames_rects in frame_rects.items(): #可遍历建值和建值对
            for frame_rect in group_frames_rects:
                right_image = tools.get_image(sheet, frame_rect['x'], frame_rect['y'], frame_rect['width'], frame_rect['height'], (0, 0, 0),
